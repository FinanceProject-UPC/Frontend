...
export class TableFee {
  ...
  generarCuota(
    ...
    const { SegDesCF, ACF, SFCF } = new FinalFeeSchedule(
      Ncuota,
      CF,
      TEM,
      N,
      pSegDesPer,
      referencia.sFCAnterior
    );
  ...
  generate_Table(flujos?: number[]): BankFee[] {
    ...
    for (let i = 0; i <= N; i++) {
      ...
      const nuevaCuota = this.generarCuota(
        i + 1,
        PG,
        anteriorSF,
        saldoCap,
